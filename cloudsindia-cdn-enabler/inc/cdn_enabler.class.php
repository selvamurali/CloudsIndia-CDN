...
class CDN_Enabler
{
    ...
    public static function add_admin_links($wp_admin_bar) {
        ...
        $wp_admin_bar->add_menu(
            [
                ...
                'title'     => '<span class="ab-item">'.esc_html__('Purge CDN', 'clouds-cdn-enabler').'</span>',
                'meta'   => ['title' => esc_html__('Purge CDN', 'clouds-cdn-enabler')],
            ]
        );
    ...
    public static function process_purge_request($data) {
        ...
        $response = wp_remote_get( 'https://api.cloudsindia.in/zones/purge/'. $options['keycdn_zone_id'] .'.json',
            [
                'timeout' => 20,
                'headers' => [
        ...
        if ( is_wp_error( $response ) ) {
            printf(
                ...
                esc_html__('Error connecting to API - '. $response->get_error_message(), 'clouds-cdn-enabler')
            );
        ...
        if ( is_array( $response ) and is_admin_bar_showing()) {
                    ...
                    and array_key_exists('description', $json) )
            {
                printf(
                    ...
                    esc_html__($json['description'], 'clouds-cdn-enabler')
                );
            ...
            if ( array_key_exists($rc, $custom_messages) ) {
                printf(
                    '<div class="notice notice-error is-dismissible"><p>%s</p></div>',
                    esc_html__('HTTP returned '. $rc .': '.$custom_messages[$rc], 'clouds-cdn-enabler')
                );
            ...
            if ( is_array($json)
                ...
                printf(
                    '<div class="notice notice-error is-dismissible"><p>%s</p></div>',
                    esc_html__('HTTP returned '. $rc .': '.$json['description'], 'clouds-cdn-enabler')
                );
            } else {
        ...
        if ( version_compare($GLOBALS['wp_version'], CDN_ENABLER_MIN_WP.'alpha', '<') ) {
            show_message(
                sprintf(
                    ...
                    sprintf(
                        __("CDN Enabler is optimized for WordPress %s. Please disable the plugin or upgrade your WordPress installation (recommended).", "clouds-cdn-enabler"),
                        CDN_ENABLER_MIN_WP
                    )
                )
            );
